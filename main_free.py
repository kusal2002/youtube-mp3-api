...
app = FastAPI(
    title="Music Metadata API (Free Tier)",
    description="""
This API allows you to:

- Search for music metadata on YouTube and YouTube Music (`/search_results`)
...
**Note:** Streaming features require FFmpeg (available on paid plans only).
Only metadata is saved to Supabase, not audio files.
""",
    version="1.0.0-free"
)
...
async def root():
    return {
        ...
        "version": "1.0.0-free",
        "features": ["search", "playlists", "user_auth"],
        "note": "Streaming requires paid plan with FFmpeg"
    }

@app.get("/search_results", summary="Search for multiple tracks (metadata only)", tags=["Search"])
async def search_results(
    ...
    """
    Search YouTube for tracks and return metadata (title, channel, duration, video_id, url, thumbnail).
...
async def get_playlist_info(url: str = Query(..., description="YouTube Music playlist URL")):
    ...
    Returns playlist details and a list of tracks with metadata.
    """
    try:
        ydl_opts = {
            'extract_flat': True,
